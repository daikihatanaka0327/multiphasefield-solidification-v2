...
run_singlemode.py
=================
Validation mode: single-crystal solid-liquid interface growth.

Purpose
-------
Verify the phase field solver in the simplest possible setting:
  - A single crystal grows into an undercooled melt.
  - The initial interface is flat and horizontal (y-direction only).
  - There are no grain boundaries in the x-direction.

What this mode checks:
  * Correct tanh initialisation of the solid-liquid interface.
  * Anisotropic surface energy  ε²(θ)  working correctly.
  * Kinetic anisotropy  b(θ)  affecting the growth rate.
  * Torque term (A11) not breaking the interface morphology.
  * Faceted growth pattern consistent with the crystal orientation.

Phases
------
  gid = 0 : liquid   (always; dummy quaternion [0, 0, 0, 1])
  gid = 1 : solid    (orientation set via config singlemode block)
  number_of_grain = 2

Output
------
  result/singlemode/step_0.png
  result/singlemode/initial_phase_map.png
  result/singlemode/initial_temperature.png
  result/singlemode/step_{N}.png  (every save_every steps)

Config additions (config.yaml → singlemode block)
--------------------------------------------------
  singlemode:
    seed_height: 32
    orientation_type: "euler"        # "euler" or "quaternion"
    euler_deg: [0.0, 0.0, 0.0]      # rotation order "xyz", degrees
    # quaternion: [x, y, z, w]      # alternative to euler_deg
"""
...
# singlemode-specific settings
sm_cfg      = cfg.get("singlemode", {})
seed_height = int(sm_cfg.get("seed_height", cfg["seed"].get("height", 32)))
out_dir     = "result/singlemode"
...
# ─── Grain orientations ───────────────────────────────────────────────────────
# singlemode: number_of_grain = 2  (liquid gid=0, solid gid=1)
...
grain_quaternions[1] = build_quaternion_from_config(sm_cfg)  # solid crystal
...
phi_cpu  = init_singlemode_phi(nx, ny, dy, delta, seed_height, solid_gid=1)
temp_cpu = init_temperature_field(nx, ny, T_melt, G, dy, seed_height)


# ─── Startup log ─────────────────────────────────────────────────────────────

print("Mode        : singlemode")
...
save_phase_map(phi_cpu, out_dir, "step_0.png",
               number_of_grain, title="singlemode — step 0")
save_phase_map(phi_cpu, out_dir, "initial_phase_map.png",
               number_of_grain, title="singlemode — Initial Phase Map")
...
for nstep in range(1, nsteps + 1):
    ...
    if nstep % save_every == 0:
        ...
        save_phase_map(current_phi, out_dir, f"step_{nstep}.png",
                       number_of_grain, title=f"singlemode — step {nstep}")
